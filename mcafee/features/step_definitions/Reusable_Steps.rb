...
# Verify a section over page
Then(/^I should see content "([^"]*)"$/) do |content_text|
  sleep 5
  if page.has_xpath?('//*[contains(text(),"'"#{content_text}"'")]')
    puts "The content - #{content_text} - is present over the page"
    sleep 5
  else
    writeFailure "FAIL - The content - #{content_text} - is NOT present over the page"
    #fail(ArgumentError.new('fail to click Cancel button'))
  end
end
